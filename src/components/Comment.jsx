...
const Comment = ({data, currentUser, globalData}) => {
  ...
  const deleteComment = () => {
    const updatedComments = globalData.comments.filter(comment => comment.id !== data.id)
    globalData.comments = updatedComments;
  ...
  return (
    <div className='comment__wrapper'>
      ...
      {reply ? 
        <ReplyComment width={100} currentCommentText={data} currentComment={data} globalData={globalData}/>
        : 
        <></>
      }
      {data.replies.length > 0 ? 
        <div className='reply__wrapper'>
          ...
          {data.replies.map((item) => 
            <CommentReplies currentUser={currentUser} key={item.id} data={item} globalData={globalData} currentComment={data}/>
          )}
        </div>
        : 
        <>
